package com.example.kevengao.godairy.adapter;
...
public class MainAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if ("0".equals(list.get(position).getLocktype())) {
			...
			/*
			* ^表示以什么开始（起始符），
			* \w表示字母、数字、下划线，+表示前面的至少出现一次，
			* 就是说以字母、数字、或下划线开头并且存在一个或者多个（a,b,ab,a_b，a123_b等等都符合）
			* ,@ 就是@了，然后\w+同理，\.表示小数点(反斜杆\这时充当转义字符,
			* 如果没反斜杆就一个点表示任意字符），
			* 【a-zA-Z]表示所有的字母包括大小写，
			* 跟随其后的{2，3}表示它前面的表达式[a-zA-Z]匹配两次到3次，
			* 也就说小数点后要有2个到3个字母存在，后面{1，2}的同理，
			* 表示(\.[a-zA-Z]{2,3})匹配1次或2次，$是结束符，
			* 所以reg所匹配到的字符串一定是以字母作为结尾的。
			*
			* */
			...
			while (m.find()) {
				...
				// 取出路径的后缀
				String type = path.substring(path.length() , path.length());
				Bitmap bm = null;
				Bitmap rbm = null;
